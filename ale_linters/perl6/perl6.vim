...
function! ale_linters#perl6#perl6#GetCommand(buffer) abort
    return "RAKUDO_EXCEPTIONS_HANDLER='JSON' "
    \   .  ale#Escape(ale_linters#perl6#perl6#GetExecutable(a:buffer))
    \   . ' '
...
function! ale_linters#perl6#perl6#Handle(buffer, lines) abort
    ...
    for l:error in values(l:errors)
        let l:col = has_key(l:error, 'pre') && l:error['pre'] != v:null
        \    ? len(l:error['pre']) + 1 : v:null

        let l:lnum = get(l:error, 'line', v:null)

        let l:text = get(l:error, 'message', v:null)
